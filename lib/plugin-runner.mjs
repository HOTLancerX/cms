...
// ─── Filesystem helpers ───────────────────────────────────────────────────────
function folderName(nx) {
    return (nx ?? "").trim().toLowerCase().replace(/\s+/g, "-");
}
...
async function main() {
    ...
    if (pending.length > 0) {
        ...
        for (const plugin of pending) {
            ...
            const folder = folderName(nx || name);
    ...
    const missing = installedPlugins.filter((p) => {
        ...
        const targetDir = path.join(PLUGIN_DIR, folderName(p.nx || p.name));
    ...
    if (missing.length > 0) {
        ...
        for (const plugin of missing) {
            ...
            const targetDir = path.join(PLUGIN_DIR, folderName(nx || name));
            const repoUrl = catalog.get(nx);
