...
function syncPluginAssets() {
    console.log("[plugin-runner] Syncing plugin assets to public folder...");
    const publicPluginDir = path.join(PROJECT_ROOT, "public", "plugin");
    ...
    for (const folder of pluginFolders) {
        const pluginIconDir = path.join(PLUGIN_DIR, folder, "icon");
        if (fs.existsSync(pluginIconDir)) {
            const destDir = path.join(publicPluginDir, folder, "icon");
            copyFolderSync(pluginIconDir, destDir);
            console.log(`[plugin-runner] Copied icons for "${folder}"`);
        }
    }
}
...
async function main() {
    ...
    const isVercelBuild = process.env.VERCEL === "1" || process.env.CI === "1" || process.env.NODE_ENV === "production";
    const cmsOriginUrl = process.env.CMS_ORIGIN_URL || (isVercelBuild ? "https://github.com/HOTLancerX/cms.git" : undefined);
    if (cmsOriginUrl) {
        ...
        if (fs.existsSync(gitDir)) {
            try {
                ...
                // If running in Vercel or CI environment, pull/reset to origin to get the latest updates
                if (isVercelBuild) {
                    console.log(`[plugin-runner] Pass 0 — Vercel/CI environment detected, fetching latest updates from origin...`);
                    execSync("git fetch origin", { cwd: PROJECT_ROOT, stdio: "inherit" });

                    let branch = "main";
                    try {
                        execSync("git rev-parse --verify origin/main", { cwd: PROJECT_ROOT, stdio: "ignore" });
                    } catch {
                        branch = "master";
                    }

                    execSync(`git reset --hard origin/${branch}`, { cwd: PROJECT_ROOT, stdio: "inherit" });
                    console.log(`[plugin-runner] Pass 0 — successfully reset and updated local branch to origin/${branch}`);
                }
            } catch (err) {
                console.warn(`[plugin-runner] Pass 0 — could not update or pull git remote: ${err.message}`);
            }
        } else {
            console.log("[plugin-runner] Pass 0 — no .git directory (zip/Vercel/Docker deploy).");
            // If running in Vercel/CI environment, download the tarball of the upstream CMS_ORIGIN_URL and extract it
            if (isVercelBuild) {
                const githubMatch = cmsOriginUrl.match(/github\.com\/([^/]+)\/([^/.]+)/);
                if (githubMatch) {
                    ...
                    for (const branch of ["main", "master"]) {
                        try {
                            ...
                            console.log(`[plugin-runner] Pass 0 — Vercel/CI environment detected. Fetching latest ${branch} branch from ${tarballUrl}...`);

                            const archivePath = path.join(PROJECT_ROOT, "cms-update.tar.gz");
                            execSync(`curl -sL "${tarballUrl}" -o "${archivePath}"`, { cwd: PROJECT_ROOT });

                            if (fs.existsSync(archivePath) && fs.statSync(archivePath).size > 0) {
                                console.log(`[plugin-runner] Pass 0 — extracting tarball updates directly over the project...`);
                                execSync(`tar -xzf "${archivePath}" --strip-components=1`, { cwd: PROJECT_ROOT });
                                fs.unlinkSync(archivePath);
                                console.log(`[plugin-runner] Pass 0 — successfully updated files directly from ${cmsOriginUrl} (${branch})`);
                                break;
                            }
                        } catch (err) {
                            console.warn(`[plugin-runner] Pass 0 — failed to update using branch ${branch}: ${err.message}`);
                        }
                    }
                } else {
                    console.warn(`[plugin-runner] Pass 0 — CMS_ORIGIN_URL is not a GitHub URL, cannot perform tarball sync.`);
                }
            } else {
                console.log("[plugin-runner] Pass 0 — skipped auto-sync in local development environment.");
            }
        }
    } else {
    ...
    if (process.argv.includes("--watch")) {
        console.log("[plugin-runner] Watch mode enabled. Watching for changes in plugin assets...");
        try {
            fs.watch(PLUGIN_DIR, { recursive: true }, (eventType, filename) => {
                if (filename) {
                    if (filename.includes("icon")) {
                        console.log(`[plugin-runner] Icon change detected: ${filename}. Syncing plugin assets...`);
                        try {
                            syncPluginAssets();
                        } catch (err) {
                            console.error("[plugin-runner] Error syncing assets:", err.message);
                        }
                    } else if (filename.endsWith("package.json")) {
                        console.log(`[plugin-runner] package.json change detected: ${filename}. Syncing dependencies...`);
                        try {
                            syncLocalPluginDeps();
                        } catch (err) {
                            console.error("[plugin-runner] Error syncing deps:", err.message);
                        }
                    }
                }
            });
        } catch (err) {
            console.error("[plugin-runner] Watcher failed to start:", err.message);
        }
