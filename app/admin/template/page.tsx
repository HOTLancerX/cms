...
interface TemplateEntry {
    // Identity — from hook registry OR builder
    key: string;
    type: string;
    label: string;
    position: number;
    ...
    // Builder-page origin (null for hook-based templates)
...
// Builder page shape returned by GET /builder
...
export default function TemplateManager() {
    ...
    useEffect(() => {
        (async () => {
            ...
            // 5. Fetch the DB list once — to know which isDefault
            ...
            // 7. Fetch builder pages that have a templateType set
            let builderTemplates: TemplateEntry[] = [];
            try {
                const builderRes = await xFetch("/builder", { cache: "no-store" });
                const builderDocs: BuilderDoc[] = await builderRes.json();
                builderTemplates = builderDocs
                    .filter((d) => d.status === "active" && d.templateType)
                    .map((d) => {
                ...
                // builder fetch failed — proceed without builder templates
            }

            // 8. Merge and sort
            const merged: TemplateEntry[] = [...hookTemplates, ...builderTemplates].sort(
        ...
        try {
            ...
            await xFetch("/template", {
                ...
                body: JSON.stringify({
                    ...
                    active: false,
                    // Carry the builderId so the layout can use it
                    builderId: tpl.builderId ?? undefined,
                }),
            });

            // Re-fetch to get the _id if we don't have it yet
            let dbId = tpl.dbId;
            if (!dbId) {
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-white/60 text-xs font-mono truncate mt-0.5">
                                            {isBuilderPage ? "Builder Page" : tpl.pluginNx}
                                        </p>
                                    </div>
                                    {tpl.isDefault && (
